...
<body>
  ...
  <header>
    ...
    <div class="controls">
      ...
      <input id="minPrice" type="number" placeholder="Min $" style="max-width: 100px;" />
      <input id="maxPrice" type="number" placeholder="Max $" style="max-width: 100px;" />
      <select id="brand">
  ...
  <script>
    ...
    function normalizedCondition(part) {
      ...
      if (text.includes("refurbished") || text.includes("refurb")) return "Refurbished";
      
      // Then check for open box
      ...
      // Check raw condition value
      if (raw === "Refurbished") return "Refurbished";
      if (raw === "Open Box") return "Open Box";
    ...
    const state = {all: [], partType: "ALL", site: "ALL", condition: "ALL", sort: "price-asc", search: "", showRaw: true, minPrice: "", maxPrice: "", brand: "ALL"};
    ...
    function applyFilters(items) {
      ...
      const minP = state.minPrice ? parseFloat(state.minPrice) : null;
      const maxP = state.maxPrice ? parseFloat(state.maxPrice) : null;
      ...
      let out = items.filter(p => {
        ...
        if (state.brand !== "ALL") {
          ...
          if (!titleLower.includes(state.brand.toLowerCase())) return false;
        }
    ...
    function wireControls() {
      ...
      document.getElementById("brand").addEventListener("change", (e) => { state.brand = e.target.value; render(); });
    }
